...
public class Parser {
	...
	public static void mapCsvToChunk(ConcurrentHashMap<String, Chunk> chunksOfOurFiles, 
			ConcurrentHashMap<String, Chunk> theirChunks, 
			ConcurrentHashMap<String, Integer> numberOfChunks) throws FileNotFoundException {
		...
		HashSet<String> file_ids = new HashSet<String>();

		for( Entry<String, Integer> filename : numberOfChunks.entrySet()){
			try {
				file_ids.add(Config.toSHA256(filename.getKey()));
		...
		for (Object object : list) {
			...
			if(file_ids.contains(chunk.getFileId() ))
				chunksOfOurFiles.put(fileIdChunkNr, chunk);
			else
				theirChunks.put(fileIdChunkNr, chunk);
		}
	}
